...
<div class="container mx-auto mt-10 dark:bg-gray-800">
    ...
    <div class="flex shadow-md my-10">
        <div class="w-3/4 bg-white px-10 py-10 dark:bg-gray-900">
            ...
            <h1 class="font-semibold text-2xl text-gray-100">Shopping Cart</h1>
            <h2 class="font-semibold text-2xl dark:text-white">{{$total_items}} item(s)</h2>
            </div>
            ...
            @php
                $total_items += $ci->qty;
                $total_cost_sp += $ci->product->price_sp * $total_items;
                $total_cost_mp += $ci->product->price_mp * $total_items;
            @endphp

            <div class="flex items-center hover:bg-gray-700 -mx-8 px-6 py-5 bg-gray-800">
                <div class="flex w-2/5"> <!-- product -->
                    <div class="w-20">
                    <a href="/products/{{$ci->product->slug}}">
                        <img class="h-24" src="{{$ci->product->image_url}}" alt="{{$ci->product->title}}">
                        </div>
                        <div class="flex flex-col justify-between ml-4 flex-grow">
                        <span class="hover:text-indigo-700 font-bold text-sm dark:text-white">{{$ci->product->title}}</span>
                        <div class="flex justify-start items-center gap-x-8">
                            <p class="mt-1 text-sm text-gray-200">
                                Color - {{ $ci->color->color }}
                            </p>
                            <div class="w-8 h-4 inline-block -ml-4" style="background-color: #{{$ci->color->code}}"></div>
                        </div>
                        <p class="mt-1 text-sm text-gray-200">
                            Size - {{$ci->size->size}}
                        </p>
                    ...
                    <form method="post" action="/cart/{{$ci->id}}">
                        @method('DELETE')
                        @csrf
                ...
                <div class="flex justify-center w-1/5">
                    ...
                    <form action="/cart/{id}" method="post">
                        ...
                        <div class="flex justify-center items-center">
                            ...
                            <input class="mx-2 border text-center w-20 dark:bg-gray-800 dark:text-white text-xl" name="qty"   type="number" min="1" max="5" value="{{$ci->qty}}">
                ...
                <span style="text-decoration: line-through" class="text-center w-1/5 font-semibold text-lg dark:text-white">&#8377;&nbsp;{{number_format($ci->product->price_mp)}}</span>
                <span class="text-center w-1/5 font-semibold text-lg dark:text-white">&#8377;&nbsp;{{number_format($ci->product->price_sp)}}</span>
                @php
                    $discount_percent = round(($ci->product->price_mp - $ci->product->price_sp)/ $ci->product->price_mp * 100, 2);
                @endphp
        ...
        <div id="summary" class="w-1/4 px-8 py-10 dark:bg-gray-900">
            ...
            @else
                ...
                <form action="/cart/remove-coupon" method="post">
                    ...
                    <div class="max-w-sm p-6 mt-8 bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700">
                        ...
                        <div class="flex justify-between text-base font-medium text-green-500">
                        <p>Code: </p>
                        <input type="hidden" value="{{$cart->coupon->id}}">
                        <p>{{$cart->coupon->coupon}}</p>
                        </div>
                        <div class="flex justify-between text-base font-medium text-green-500">
                            <p>Discount </p>
            ...
            <p class="text-green-400 text-lg -mt-4 mb-4">You will save &#8377; {{number_format($order->discount)}}</p>
